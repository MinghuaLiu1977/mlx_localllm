...
* **优化日志输出**: 所有的 `print`、`debugPrint` 和 `NSLog` 语句现在都已使用 `kDebugMode` (Dart) 或 `#if DEBUG` (Swift) 进行包裹，确保它们仅在调试模式下打印，提升了 Release 模式下的性能并减少了日志污染。

# 0.2.12

* **详尽文档更新**: 大幅扩展了 `README.md`，增加了详细的 API 参考、参数表格以及推理模式指南。为 `lib/mlx_localllm.dart` 添加了全量的 DartDoc 注释，以获得更好的 IDE 开发支持。
